package net.akarisakai.fantasyisekaimod.entity;

import net.minecraft.world.entity.LivingEntity;
import net.minecraft.world.entity.Mob;
import net.minecraft.world.entity.ai.goal.Goal;
import net.minecraft.world.phys.Vec3;

import java.util.EnumSet;

public class DirewolfLeapAtTargetGoal extends Goal {
    private final Mob mob;
    private LivingEntity target;
    private final float yd;
    private long lastUseTime; // To track the last time the goal was used.
    private final long cooldown = 7 * 20; // Cooldown in ticks (7 seconds).

    public DirewolfLeapAtTargetGoal(Mob pMob, float pYd) {
        this.mob = pMob;
        this.yd = pYd;
        this.setFlags(EnumSet.of(Flag.JUMP, Flag.MOVE));
    }

    public boolean canUse() {
        if (this.mob.isVehicle()) {
            return false;
        } else {
            // Check if enough time has passed since the last use.
            if (System.currentTimeMillis() - lastUseTime < cooldown) {
                return false;
            }

            this.target = this.mob.getTarget();
            if (this.target == null) {
                return false;
            } else {
                double $$0 = this.mob.distanceToSqr(this.target);
                if (!($$0 < 4.0) && !($$0 > 16.0)) {
                    if (!this.mob.onGround()) {
                        return false;
                    } else {
                        return this.mob.getRandom().nextInt(reducedTickDelay(5)) == 0;
                    }
                } else {
                    return false;
                }
            }
        }
    }

    public boolean canContinueToUse() {
        return !this.mob.onGround();
    }

    public void start() {
        Vec3 $$0 = this.mob.getDeltaMovement();
        Vec3 $$1 = new Vec3(this.target.getX() - this.mob.getX(), 0.0, this.target.getZ() - this.mob.getZ());
        if ($$1.lengthSqr() > 1.0E-7) {
            $$1 = $$1.normalize().scale(0.4).add($$0.scale(0.2));
        }

        this.mob.setDeltaMovement($$1.x, (double)this.yd, $$1.z);

        // Record the time when this goal was last used.
        lastUseTime = System.currentTimeMillis();
    }
}